#!/usr/bin/env python3
"""
Commit Staged Changes - Create Git commits with AI contribution statistics for staged changes only.
...
Unlike commit_with_stats.py, this script will NOT auto-stage changes.
It only commits what you've already staged.

Usage:
    python commit_staged.py
    python commit_staged.py --message "Custom commit message"
"""
...
def run_command(cmd, capture_output=True):
    ...
    """
    try:
        # bandit: disable=B603 (Test for use of subprocess without shell equals true)
...
def main():
    parser = argparse.ArgumentParser(
        description='Create a Git commit with AI contribution statistics for staged changes only.'
    )
    parser.add_argument(
        '--message', '-m',
        type=str,
    ...
    # Check for staged changes - DO NOT auto-stage anything
    staged = run_command(["git", "diff", "--cached", "--name-only"])
    if not staged:
        print("❌ No changes are currently staged.")
        print("   Use 'git add <file>' to stage changes before committing.")
        print("   Or use the 'commit-changes' skill to auto-stage and commit all changes.")
        sys.exit(0)
